# 告别反射、N+1 与 VO 膨胀！EasyTrans：超高性能关联 ID 自动批量就地翻译框架正式开源！

在企业级 Java 开发（如电商、金融、OA、ERP 等系统）中，我们几乎每天都会遇到 **关联数据转义** 的典型场景。

例如：数据库实体 `OrderPO` 中存的是 `userId`、`goodsId`。而在返回给前端的对象中，我们需要将其翻译（填充）为 `userName`、
`goodsName`。
...
3. **基于反射/动态代理的运行时转义**：虽然用注解省了事，但在高并发、大数量级翻译时，频繁的反射开销会导致 CPU 飙升与高延迟。
4. **繁重的 PO ➡️ VO 拷贝（VO 膨胀）**：为了使用传统的 MapStruct 等拷贝框架，我们不得不为每个 PO 写一个大面积重复的 VO
   伴生类。项目充斥着上百个无意义的 `XXXVO`、`XXXDTO`，类爆炸、极难维护！且 Controller 签名声明返回 PO，序列化出去时却被切面硬生生替换成
   VO，契约不严谨，极其不专业！

---

## 🚀 EasyTrans 2.0：就地翻译 (In-place Translation) 带来极限性能与极简开发

**EasyTrans 2.0** 另辟蹊径，带来划时代的**就地翻译 (In-place Translation)** 模式。
你再也不用定义繁琐的 VO 类，也不需要引入 MapStruct 注解和多余的拷贝插件！直接在原实体对象（PO / Entity / DTO）上进行就地转义填充。

EasyTrans 在**编译期**通过 APT 自动生成原生、高吞吐的桥接类（Bridge）。所有的 ID 收集、嵌套递归和属性回填代码在运行期等同于你一行行手写的纯
Java 代码，物理性能达到绝对极限，彻底解放你的项目依赖与代码体积。
...
EasyTrans **100% 零反射、零动态代理、零拷贝**。在你的项目编译时，APT（编译期注解处理器）会自动为你生成极其纯净、高吞吐的原生桥接类。ID
...
3. **第二阶段（硬编码回填）**：将批量拉取到的数据，通过纯硬编码的 setter 方法就地秒级回填到实体的对应转义属性中。
...
在展示文本属性上配置，通过 `source` 指定包含关联 ID 的属性，通过 `type` 指定翻译对应的 Feeder 标识。
...
如果你厌倦了冗长的 VO 拷贝、反胃的 N+1 面条代码，或是因反射导致的高并发延迟，**EasyTrans** 将是为你量身定制的破局利器。
